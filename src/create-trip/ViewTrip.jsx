...
export const hotelIcon = L.divIcon({
  ...
  html: `<div style="background-color: #9333ea; width: 36px; height: 36px; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: 3px solid white; box-shadow: 0 4px 6px rgba(0,0,0,0.3); font-size: 18px;">🏨</div>`,
  iconSize: [36, 36],
  iconAnchor: [18, 18],
  popupAnchor: [0, -18]
});

export const rentalIcon = L.divIcon({
  className: 'custom-div-icon',
  html: `<div style="background-color: #ea580c; width: 36px; height: 36px; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: 3px solid white; box-shadow: 0 4px 6px rgba(0,0,0,0.3); font-size: 18px;">🚗</div>`,
  iconSize: [36, 36],
  iconAnchor: [18, 18],
  popupAnchor: [0, -18]
});
  ...
  const handleBookingSubmit = async (bookingData) => {
    try {
      ...
      if (response.data.success) {
        alert('Booking requested securely! Your request has been sent to the property owner for approval.');
          ...
          >
            ...
            <div className="flex-1 rounded-xl overflow-hidden border-2 border-gray-100">
              <MapContainer center={[20.59, 78.96]} zoom={5} style={{ height: "100%", width: "100%" }}>
                ...
                {/* --- HOTEL MARKER --- */}
                {trip.businessDetails?.hotel?.address && (
                  <Marker
                    position={[
                      trip.businessDetails.hotel.geo?.lat || 20.59,
                      trip.businessDetails.hotel.geo?.lng || 78.96
                    ]}
                    icon={hotelIcon}
                  >
                    <Tooltip permanent className="font-bold text-purple-700">Hotel</Tooltip>
                    <Popup>
                      <div className="p-2">
                        <strong className="text-purple-700">🏨 {trip.businessDetails.hotel.name}</strong>
                        <div className="text-sm text-gray-600 mt-1">
                          {trip.businessDetails.hotel.address}
                        </div>
                      </div>
                ...
                {/* --- RENTAL MARKER --- */}
                {trip.businessDetails?.rental && (
                  <Marker
                    position={[
                      trip.businessDetails.rental.geo?.lat || 20.69,
                      trip.businessDetails.rental.geo?.lng || 78.86
                    ]}
                    icon={rentalIcon}
                  >
                    <Tooltip permanent className="font-bold text-orange-600">Rental</Tooltip>
                    <Popup>
                      <div className="p-2 border-b">
                        <strong className="text-orange-600">🚗 {trip.businessDetails.rental.name}</strong>
                        <div className="text-sm text-gray-600 mt-1">
                          {trip.businessDetails.rental.vehicleType} - {trip.businessDetails.rental.model}
                        </div>
                      </div>
                    </Popup>
                  </Marker>
